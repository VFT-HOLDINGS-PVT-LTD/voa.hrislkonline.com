...
class Welfair extends CI_Controller {
    ...
    public function index() {
        ...
        $data['data_alw'] = $this->Db_model->getData('ID,Welfair_name', 'tbl_welfair');
        $this->load->view('Payroll/Welfair/index', $data);
    }
    ...
    public function insert_data() {
        ...
        if ($fixed == 1) {
            $IsfixedStamp = $this->Db_model->getfilteredData("select count(EmpNo) HasRow from tbl_variable_welfair where EmpNo ='$Emp' and welfair_id='$allowance' and month='0' and Year = '0' ");
            if ($IsfixedStamp[0]->HasRow > 0) {
                ...
                for ($i = 0; $i < $Count; $i++) {
                    ...
                    $this->db->insert_batch('tbl_variable_welfair', $data);
                }
                $this->session->set_flashdata('success_message', 'Fixed Welfair added successfully');
            }
        }else{
            $IsAllowance = $this->Db_model->getfilteredData("select count(EmpNo) HasRow from tbl_variable_welfair where EmpNo ='$Emp' and welfair_id='$allowance' and month='$month' and Year = '$year' ");

            if ($IsAllowance[0]->HasRow > 0) {
                $this->session->set_flashdata('error_message', 'Already Added This Welfair for This Employee');
            } else {
                for ($i = 0; $i < $Count; $i++) {
                    $data = array(
                        array(
                ...
                $this->session->set_flashdata('success_message', 'Stamp Welfair successfully');
    ...
    public function get_details() {
        ...
        $dataObject = $this->Db_model->getfilteredData("SELECT 
                                                                ...
                                                                tbl_variable_welfair v_alw
                                                                    ...
                                                                    tbl_departments dep ON dep.Dep_id = Emp.Dep_id
                                                                        LEFT JOIN
                                                                        tbl_welfair alw_typ ON alw_typ.ID = v_alw.welfair_id where v_alw.ID=$id
                                                                    ");
    ...
    public function edit() {
        ...
        $welfair = $this->input->post("welfair", TRUE);
        $amount = $this->input->post("amount", TRUE);

        $data = array("Amount" => $amount);
        $whereArr = array("ID" => $ID);
        $result = $this->Db_model->updateData("tbl_variable_welfair", $data, $whereArr);
        
        $this->session->set_flashdata('success_message', 'Welfair edit successfully');
