...
class Late_Deduction_Report extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('Reports/Payroll/Late_Deduction_Report', $data);
    }
    ...
    public function Attendance_Report_By_Cat() {
        ...
        if (($this->input->post("txt_from_date")) && ($this->input->post("txt_to_date"))) {
            if ($filter == '') {
                $filter = " where  ir.Date between '$from_date' and '$to_date'";
            } else {
                $filter .= " AND  ir.Date between '$from_date' and '$to_date'";
            }
        }
        ...
        $data['data_set'] = $this->Db_model->getfilteredData("SELECT 
                                                                ...
                                                                FROM
                                                                    ...
                                                                    {$filter}  order by Emp.EmpNo,ir.Date ASC");

//        var_dump($data);die;

        $this->load->view('Reports/Payroll/rpt_late_deduction', $data);
    }
