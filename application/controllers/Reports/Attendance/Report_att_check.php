...
class Report_att_check extends CI_Controller
{
    ...
    public function Attendance_Report_By_Cat()
    {
        ...
        ");
                // $query = $this->db->query($attendance);
                // $attendance = $query->result_array();

                // To echo the SQL query:
                ...
                if (!empty($attendance)) {
                    ...
                    //     if($inDay == $outDay){
                    //         if($record->OutTime != "00:00:00"){
                    //             $employeeAttendance[$inDay]->OutTime = $record->OutTime;
                    ...
                    //     // echo "<br>";
                    //     // // print_r($employeeAttendance[$outDay]);
                    //     // // echo "<br>";
                    //     // echo "<br>";
                    // }
                    foreach ($attendance as $record) {
    ...
    // Initialize day entry if not already set
    if (!isset($employeeAttendance[$inDay])) {
        $employeeAttendance[$inDay] = new stdClass();
    }

    // Set Shift In (FTime) to the correct day
    $employeeAttendance[$inDay]->FTime = $record->FTime;
    $employeeAttendance[$inDay]->TTime = $record->TTime;

    // Set InTime to the correct day
    $employeeAttendance[$inDay]->InTime = $record->InTime;

    // Include DayStatus in the correct day entry
    $employeeAttendance[$inDay]->DayStatus = $record->DayStatus;

    // If InDate and OutDate are the same day, handle OutTime on the same day
    if($inDay == $outDay) {
        if($record->OutTime != "00:00:00"){
            $employeeAttendance[$inDay]->OutTime = $record->OutTime;
        }
    } else {
        // Handle OutDay for a different day if InDate and OutDate are different
        if (!isset($employeeAttendance[$outDay])) {
            $employeeAttendance[$outDay] = new stdClass();
        }
        $employeeAttendance[$outDay]->OutTime = $record->OutTime;
        ...
        // $data['attendance_data'] = $dept;

        
        if($dept == 'null'){
            $data['dept'] = 'null';
        }else{
            $deptData = $this->Db_model->getfilteredData("SELECT * FROM `tbl_departments` WHERE `tbl_departments`.`Dep_ID` = '$dept'");
            $data['dept'] = $deptData[0]->Dep_Name;
        }
        // $data['dept'] = (!empty($dept) && $deptData[0]->Dep_Name == 'null') ? 'null' : $deptData[0]->Dep_Name;
